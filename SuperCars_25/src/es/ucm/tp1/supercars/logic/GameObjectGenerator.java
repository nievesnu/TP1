	...
	public static void generateGameObjects(Game game, Level level) { //modificar randlane con x
		...
		for (int x = game.getVisibility() / 2; x < game.getRoadLength(); x++) {
			...
			if (level . hasAdvancedObjects()) {
				...
				game.tryToAddObject(new Wall(game, x, game.getRandomLane()), level.getAdvancedObjectsFrequency());
				game.tryToAddObject(new Turbo(game, x, game.getRandomLane()), level.getAdvancedObjectsFrequency());
				if (! SuperCoin.hasSuperCoin()) {
					game.tryToAddObject(new SuperCoin(game, x, game.getRandomLane()), level.getAdvancedObjectsFrequency());
				}
				game.tryToAddObject(new Truck(game, x, game.getRandomLane()), level.getAdvancedObjectsFrequency());
				game.tryToAddObject(new Pedestrian(game, x, 0), level.getAdvancedObjectsFrequency());
			}
		}
	}
