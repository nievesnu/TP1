...
public class Game {
	...
	public boolean tryToAddObject(int x, int y, double frequency) {
		boolean ok = false;
		if	(getRandomNumber() < frequency){
			if(oList.isPosEmpty(x, y)) { //oList como object list importada de la clase objetos
	...
	/*public void tryToAddObject(int x, int y, double frequency) {
		if	(getRandomNumber() < frequency){
